<!DOCTYPE HTML>
<html lang="en">

<head>
	<title>TABEL</title>
	<link rel="stylesheet" href="style.css" type="text/css"/>
</head>

<body>
	<div id="wrapper">
		<header>
			<img src="moneykey.png" alt="Header" width="80" height="75" />
			<div class="main-nav">
				<ul>
					<li>Home</li>
					<li>Problems</li>
					<li>Submission</li>
					<li>Rank List</li>
				</ul>
			</div>
			<div class="background"></div>
		</header>
		
		<section>
			<h1>Problems</h1>
			<table class="problem-table" border=1 cellspacing=0 cellpadding=10>
				<tr>
					<th width="30%"></th>
					<th width="25%" colspan=4>Submission</th>
					<th width="25%" colspan=3>User</th>
					<th width="10%"></th>
					<th width="10%" colspan=2></th>
				</tr>

				<tr>
					<th>Name</th>
					<th>Total</th>
					<th>Acc.</th>
					<th>Ratio</th>
					<th>Fastest</th>
					<th>Total</th>
					<th>Acc.</th>
					<th>Ratio</th>
					<th>Dificulty</th>
					<th></th>
					<th></th>
				</tr>

				<tr>
					<td>10 Kind of People</td>
					<td>78</td>
					<td>29</td>
					<td>37%</td>
					<td>0.01</td>
					<td>32</td>
					<td>28</td>
					<td>88%</td>
					<td>2.6</td>
					<td></td>
					<td></td>
				</tr>

				<tr>
					<td>ACM Contest Scoring</td>
					<td>1209</td>
					<td>655</td>
					<td>55%</td>
					<td>0.00</td>
					<td>621</td>
					<td>588</td>
					<td>95%</td>
					<td>1.4</td>
					<td></td>
					<td></td>
				</tr>

				<tr>
					<td>Adding Words</td>
					<td>1275</td>
					<td>230</td>
					<td>18%</td>
					<td>0.00</td>
					<td>312</td>
					<td>198</td>
					<td>63%</td>
					<td>5.2</td>
					<td></td>
					<td></td>
				</tr>

				<tr>
					<td>Bacon Eggs and Spam</td>
					<td>249</td>
					<td>117</td>
					<td>35%</td>
					<td>0.00</td>
					<td>109</td>
					<td>102</td>
					<td>94%</td>
					<td>1.8</td>
					<td></td>
					<td></td>
				</tr>

				<tr>
					<td>Car Game</td>
					<td>1011</td>
					<td>280</td>
					<td>28%</td>
					<td>0.18</td>
					<td>184</td>
					<td>109</td>
					<td>59%</td>
					<td>5.9</td>
					<td></td>
					<td></td>
				</tr>

				<tr>
					<td>Dungeon Master</td>
					<td>172</td>
					<td>62</td>
					<td>36%</td>
					<td>0.00</td>
					<td>62</td>
					<td>50</td>
					<td>81%</td>
					<td>3.6</td>
					<td></td>
					<td></td>
				</tr>
				
				<tr>
					<td>Eight Queens</td>
					<td>4283</td>
					<td>1084</td>
					<td>25%</td>
					<td>0.00</td>
					<td>1105</td>
					<td>864</td>
					<td>78%</td>
					<td>3.0</td>
					<td></td>
					<td></td>
				</tr>
				
				<tr>
					<td>Fundamental Neighbors</td>
					<td>93</td>
					<td>35</td>
					<td>38%</td>
					<td>0.02</td>
					<td>25</td>
					<td>21</td>
					<td>84%</td>
					<td>3.8</td>
					<td></td>
					<td></td>
				</tr>
				
				<tr>
					<td>Fun House</td>
					<td>335</td>
					<td>221</td>
					<td>66%</td>
					<td>0.00</td>
					<td>213</td>
					<td>199</td>
					<td>93%</td>
					<td>1.9</td>
					<td></td>
					<td></td>
				</tr>
				
				<tr>
					<td>Galaksija</td>
					<td>10</td>
					<td>5</td>
					<td>50%</td>
					<td>0.42</td>
					<td>5</td>
					<td>5</td>
					<td>100%</td>
					<td>5.2</td>
					<td></td>
					<td></td>
				</tr>
				
				<tr>
					<td>ICPC Tutorial</td>
					<td>2407</td>
					<td>576</td>
					<td>24%</td>
					<td>0.00</td>
					<td>647</td>
					<td>531</td>
					<td>82%</td>
					<td>2.9</td>
					<td></td>
					<td></td>
				</tr>
			</table>
		</section>

		<footer>
			<div class="md-12">
			Copyright &copy; Izzati Alvandiar. S1 Pendidikan Teknik Informatika.
			</div>
		</footer>
	</div>
</body>
</html>
